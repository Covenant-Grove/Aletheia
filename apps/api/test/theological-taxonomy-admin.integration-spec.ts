...
describe('Theological taxonomy admin API (real Postgres)', () => {
  ...
  async function registerAndGetCookie(emailPrefix: string): Promise<string> {
    ...
    const response = await supertest(app.getHttpServer())
      ...
      .send({ email, password: 'somePassword123', fullName: 'Theological Taxonomy Admin Test' })
      .expect(201);
  ...
  beforeAll(async () => {
    ...
    const adminResponse = await supertest(app.getHttpServer())
      .post('/api/v1/auth/register')
      .send({ email: adminEmail, password: 'somePassword123', fullName: 'Theological Taxonomy Admin Test' })
      .expect(201);
